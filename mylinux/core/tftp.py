...
class TFTPD:
	...
	def __init__(self, **server_settings):
		...
		self.mode_debug = server_settings.get('mode_debug', True)  # debug mode
	...
	def listen(self):
		...
		while True:
			...
			rlist, _, _ = select.select([self.sock] + [client.sock for client in self.ongoing if not client.dead], [],
			                            [], 1)
			for sock in rlist:
				if sock == self.sock:
