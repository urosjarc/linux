...
class DHCPD:
	...
	def __init__(self, **settings):
		...
		self.ip = settings.get('ip', '192.168.2.2')
		self.port = int(settings.get('port', 67))
		self.offer_from = settings.get('offer_from', '192.168.2.100')
		self.offer_to = settings.get('offer_to', '192.168.2.150')
		self.subnet_mask = settings.get('subnet_mask', '255.255.255.0')
		self.router = settings.get('router', '192.168.2.1')
		self.dns_server = settings.get('dns_server', '8.8.8.8')
		self.broadcast = settings.get('broadcast', '<broadcast>')
		self.file_server = settings.get('file_server', '192.168.2.2')
		...
		self.ipxe = settings.get('use_ipxe', False)
		self.http = settings.get('use_http', False)
		self.mode_proxy = settings.get('mode_proxy', False)  # ProxyDHCP mode
		self.static_config = settings.get('static_config', dict())
		self.whitelist = settings.get('whitelist', False)
		self.mode_verbose = settings.get('mode_verbose', False)  # debug mode
		self.mode_debug = settings.get('mode_debug', True)  # debug mode
		self.logger = settings.get('logger', None)
		self.save_leases_file = settings.get('saveleases', '')
		...
		# AR_INET       = Address family: Internet Protocol v4 addresses
		# SOCK_DGRAM    = Socket type: Datagrams
		...
		signal.signal(signal.SIGINT, self.export_leases)  # Ctrl-c
		signal.signal(signal.SIGTERM, self.export_leases)  # Request for termination
		signal.signal(signal.SIGALRM, self.export_leases)  # Time limit preceding alarm
		signal.signal(signal.SIGHUP, self.export_leases)  # Terminal closed
	...
	def craft_header(self, message):
		...
		if not self.mode_proxy:
			if self.leases[client_mac]['ip'] and self.leases[client_mac]['expire'] > time():  # OFFER
				...
				self.logger.info('New Assignment - MAC: {0} -> IP: {1}'.format(self.get_mac(client_mac),
	...
	def craft_options(self, opt53, client_mac):
		...
		if not filename:
			if not self.ipxe or not self.leases[client_mac]['ipxe']:
				...
				if 'options' in self.leases[client_mac] and 93 in self.leases[client_mac][
